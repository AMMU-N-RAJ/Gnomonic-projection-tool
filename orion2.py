...
# ─── Constants ────────────────────────────────────────────────────────────────
HEX_SIDE1 = 17.5            # mm, hexagon side length
HEX_SIDE2 = 15.2
PLATE_DIAMETER = 40.0       # mm
...
def is_inside_hexagon(x_mm, y_mm, side1, side2):
    """
    ...
    if ax > side1:
        return False
    if ay > side2 * SQRT3 / 2.0:
        return False
    if SQRT3 * ax + ay > SQRT3 * side2:
        return False
    return True


def compute_scale_factor(projected_stars, hex_side1 ,hex_side2 , margin=0.90):
    """
    ...
    def all_inside(scale):
        for x_rad, y_rad in projected_stars:
            x_mm = x_rad * scale
            y_mm = y_rad * scale
            if not is_inside_hexagon(x_mm, y_mm, hex_side1,hex_side2):
                return False
        return True
